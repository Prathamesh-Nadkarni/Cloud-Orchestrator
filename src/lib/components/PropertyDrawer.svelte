<script>
  ...
  let { selectedNode = $bindable(), selectedEdge = $bindable(), nodes = $bindable(), edges = $bindable() } = $props();
    ...
    if (selectedEdge) {
      edges = edges.map(e => {
        if (e.id === selectedEdge.id) {
          ...
          if (key === 'protocol') {
            switch(value) {
              case 'http': updatedEdge.style = "stroke: #3b82f6; stroke-width: 2; stroke-dasharray: 5 5;"; break;
              case 'https': updatedEdge.style = "stroke: #10b981; stroke-width: 2;"; break;
              case 'tcp': updatedEdge.style = "stroke: #f59e0b; stroke-width: 2;"; break;
              case 'udp': updatedEdge.style = "stroke: #8b5cf6; stroke-width: 2; stroke-dasharray: 4 8;"; break;
              default: updatedEdge.style = "stroke: var(--text-color); stroke-width: 2;"; break;
            }
          }
          return updatedEdge;
        }
...
<aside class="property-drawer glass-panel" class:open={!!selectedNode}>
  {#if selectedNode}
    <div class="drawer-header">
      <div class="header-info">
        <span class="provider-badge" style="color: var(--accent-{selectedNode.data.provider})">
    ...
    <div class="drawer-content">
      <div class="form-group">
        ...
        <input 
          ...
          placeholder="my-{selectedNode.data.label.toLowerCase().replace(/\s+/g, '-')}"
        />
      </div>
      ...
      {#if selectedNode.data.provider === 'aws' || selectedNode.data.provider === 'azure' || selectedNode.data.provider === 'gcp'}
        <div class="form-group">
          ...
          <input 
            ...
            placeholder={selectedNode.data.provider === 'aws' ? 'us-east-1' : 'East US'}
          />
        </div>
      {/if}
      ...
      {#if selectedNode.data.label.includes('Instance') || selectedNode.data.label.includes('VM') || selectedNode.data.label.includes('Engine') || selectedNode.data.type === 'kubernetes'}
        <div class="form-group">
          ...
          <select 
            id="instance-size" 
            value={selectedNode.data.size || (selectedNode.data.provider === 'aws' ? 't2.micro' : selectedNode.data.provider === 'azure' ? 'Standard_B1s' : 'e2-micro')} 
          ...
          >
            ...
            {:else if selectedNode.data.provider === 'azure'}
              <optgroup label="Burstable (B-Series)">
                <option value="Standard_B1s">Standard_B1s (1 vCPU, 1 GiB)</option>
                <option value="Standard_B2s">Standard_B2s (2 vCPU, 4 GiB)</option>
                <option value="Standard_B2ms">Standard_B2ms (2 vCPU, 8 GiB)</option>
                <option value="Standard_B4ms">Standard_B4ms (4 vCPU, 16 GiB)</option>
              </optgroup>
              <optgroup label="General Purpose (D-Series)">
                <option value="Standard_D2s_v3">Standard_D2s_v3 (2 vCPU, 8 GiB)</option>
                <option value="Standard_D4s_v3">Standard_D4s_v3 (4 vCPU, 16 GiB)</option>
                <option value="Standard_D8s_v3">Standard_D8s_v3 (8 vCPU, 32 GiB)</option>
              </optgroup>
              <optgroup label="Compute Optimized (F-Series)">
                <option value="Standard_F2s_v2">Standard_F2s_v2 (2 vCPU, 4 GiB)</option>
                <option value="Standard_F4s_v2">Standard_F4s_v2 (4 vCPU, 8 GiB)</option>
              </optgroup>
              <optgroup label="Memory Optimized (E-Series)">
                <option value="Standard_E2s_v3">Standard_E2s_v3 (2 vCPU, 16 GiB)</option>
                <option value="Standard_E4s_v3">Standard_E4s_v3 (4 vCPU, 32 GiB)</option>
              </optgroup>
            {:else}
              ...
              <optgroup label="High CPU (n1, n2)">
                <option value="n1-highcpu-2">n1-highcpu-2 (2 vCPU, 1.8 GB)</option>
                <option value="n1-highcpu-4">n1-highcpu-4 (4 vCPU, 3.6 GB)</option>
                <option value="n2-highcpu-2">n2-highcpu-2 (2 vCPU, 2 GB)</option>
              </optgroup>
              <optgroup label="High Memory (n1, n2)">
                <option value="n1-highmem-2">n1-highmem-2 (2 vCPU, 13 GB)</option>
                <option value="n2-highmem-2">n2-highmem-2 (2 vCPU, 16 GB)</option>
              </optgroup>
            {/if}
          </select>
        </div>
  ...
  {:else if selectedEdge}
    <div class="drawer-header">
      <div class="header-info">
        <span class="provider-badge" style="color: var(--accent-primary)">NETWORK RULE</span>
  ...
  {:else}
    <div class="empty-state">
      <p>Select a node or connection on the canvas to configure its properties</p>
    </div>
  {/if}
</aside>
