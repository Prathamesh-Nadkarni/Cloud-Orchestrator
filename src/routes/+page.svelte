<script lang="ts">
  ...
  const onDrop = (event: DragEvent) => {
    ...
    // Resource types that must be isolated inside a Subnet, not placed directly
    // inside a VPC/VNet.  This prevents the simulator from flagging a bypass
    // that the nesting rules themselves should have blocked.
    const REQUIRES_SUBNET = ["compute", "database", "storage", "loadBalancer"];
    ...
    if (parentContainer && !["vpc", "vnet"].includes(type)) {
      if (type === "kubernetes") {
        ...
        // Enforce subnet isolation: compute/database/storage/loadBalancer cannot
        // live directly inside a VPC or VNet — they must be inside a Subnet.
        if (
          ["vpc", "vnet"].includes(parentContainer.data.type) &&
          REQUIRES_SUBNET.includes(type)
        ) {
          canNest = false;
        } else {
          canNest = true;
    ...
    if (!canNest) {
      ...
      if (
        ...
        msg = `${type.charAt(0).toUpperCase() + type.slice(1)} resources must be placed INSIDE a Subnet, not directly inside a VPC/VNet. First add a Subnet inside the VPC/VNet, then place the resource inside that Subnet.`;
      } else if (type === "kubernetes") {
        msg =
  ...
  const handleNodeDragStop = ({
    ...
    const REQUIRES_SUBNET_DRAG = ["compute", "database", "storage", "loadBalancer"];
    ...
    if (parentContainer && canBeNested) {
      if (node.data.type === "kubernetes") {
        ...
        if (
          ["vpc", "vnet"].includes(parentContainer.data.type) &&
          REQUIRES_SUBNET_DRAG.includes(node.data.type)
        ) {
          canNest = false;
        } else {
          canNest = true;
    ...
    if (currentParentId !== targetParentId) {
      if (!canNest && targetParentId === undefined && parentContainer) {
        // Moving into an invalid container — explain why
        const nodeType = node.data.type;
        if (
          ["vpc", "vnet"].includes(parentContainer.data.type) &&
          REQUIRES_SUBNET_DRAG.includes(nodeType)
        ) {
          showAlert(
            "Placement Error",
            `${nodeType.charAt(0).toUpperCase() + nodeType.slice(1)} resources must be inside a Subnet, not directly inside a VPC/VNet. Place it inside a Subnet first.`,
            "error",
          );
          return;
        }
